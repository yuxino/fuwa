...
- In `CHANGELOG.md`, nest `### English` and `### 中文` under each version heading so the release extractor includes both languages. The top-level version headings remain unchanged.
