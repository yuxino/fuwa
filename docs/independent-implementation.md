...
Fuwa is an original, independently implemented macOS application distributed under the MIT License.
...
Fuwa does not use private WindowServer APIs, copy code from another application, inject code into other processes, or require changes to System Integrity Protection.
...
The projects have different implementation boundaries: Fuwa uses public ScreenCaptureKit APIs to create its own live mirror panels. Its optional `Interact` action only activates and raises the real source window through public Accessibility APIs; it does not inject input or modify the source window's WindowServer level.
...
Fuwa 是一款原创、独立实现并以 MIT 许可证发布的 macOS 应用。其代码依据 Apple 公开 API 与文档、项目原创设计和测试编写。
...
Fuwa 通过公开的 ScreenCaptureKit API 创建自有镜像面板；可选的 `Interact` 仅通过公开辅助功能 API 激活并抬升真实源窗口，不注入输入，也不改变源窗口的 WindowServer 层级。Topit 及其作者不赞助、背书或参与 Fuwa。
